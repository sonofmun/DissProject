...
paths = paths = [r'C:\CompLing\HBible']#, r'C:\Users\matthew.munson\Google Drive\Dissertation\CompLing\GBible\UTF8\ChunkedLXX']
filenames = sorted([os.path.join(path, fn) for path in paths for fn in os.listdir(path) if fn.endswith('.txt')])
vectorizer = text.CountVectorizer(input='filename', min_df=20) #stop_words = StopList, min_df=20)
...
with open(r'C:\CompLing\HBible\HBTops.txt', mode = 'w+', encoding = 'utf-8') as TopicFile:
