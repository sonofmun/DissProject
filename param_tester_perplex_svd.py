...
def svd_calc(df, e=.25):
    ...
    S = np.diag(s)
    svd_array = np.dot(U, S**e)
    return pd.DataFrame(svd_array, index=df.index, columns=df.columns)
        ...
        for size in range(min_w, max_w+1):
            for weighted in (True, False):
                ...
                for train, test in kf:
                                    ...
                                    (np.log
                                        ...
                                        (scaler
                                         ...
                                         t_test.ix[ind_int,ind_int])
                                          .values))
                                    * exponent))
                    t_ll_svd = svd_calc(t_ll)
                    del t_ll
                    ll_svd_list.append(pow
                                   (np.e,
                                    np.sum
                                    (np.log
                                        (1/np.multiply
                                        (scaler
                                         (t_ll_svd).ix[ind_int,ind_int],
                                         t_test.ix[ind_int,ind_int])
                                          .values))
                                    * exponent))
                    ...
                    del t_pmi
                    pmi_svd_list.append(pow
                                   (np.e,
                                    np.sum
                                    (np.log
                                        (1/np.multiply
                                        (scaler
                                         (t_pmi_svd).ix[ind_int,ind_int],
                                         t_test.ix[ind_int,ind_int])
                                          .values))
                                    * exponent))
                    counter += 1
                ...
                perplex_dict[('LL_SVD',
                              ...
                              'lems=%s' % (lemmata),
                              'weighted =%s' % (weighted))] = \
                                sum(ll_svd_list)/len(ll_svd_list)
                perplex_dict[('PPMI_SVD',
                              size,
                              'lems=%s' % (lemmata),
                              'weighted =%s' % (weighted))] = \
                                sum(pmi_svd_list)/len(pmi_svd_list)
