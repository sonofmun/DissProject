...
class CatSimWin(CatSim):
	...
	def WriteFiles(self):
		with open('{2}/LN_Word_Cat_Scores_{0}_rng={1}.pickle'.format(self.algo, self.rng, self.dest_dir), mode='wb') as file:
		...
		for w_size in self.scores.keys():
			save_file = '{3}/LN_Window={0}_Word_Cat_Scores_SVD_exp={1}_{2}.csv'.format(str(w_size), 'None', self.algo, self.dest_dir)
			self.WriteLines(save_file, w_size, 'None', lems)
		with open('{2}/LN_Window_Averages_{0}_rng={1}.pickle'.format(self.algo, self.rng, self.dest_dir), mode='wb') as file:
			dump(self.averages, file)
		with open('{2}/LN_Window_Averages_{0}_rng={1}.csv'.format(self.algo, self.rng, self.dest_dir),
				  mode='w',
		...
		with open('{2}/LN_Window_Averages_no_93_SVD_{0}_rng={1}.pickle'.format(self.algo, self.rng, self.dest_dir), mode='wb') as file:
			dump(self.ave_no_93, file)
		with open('{2}/LN_Window_Averages_no_93_SVD_{0}_rng={1}.csv'.format(self.algo, self.rng, self.dest_dir),
				  mode='w',
				  encoding='utf-8') as file:
			file.write('Average Number of Standard Deviations above or below Average '
