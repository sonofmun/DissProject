...
class get_local:
	...
	def construct_dicts(self):
		with open('{}/canonical-greekLit/canonical-greekLit.tracking.json'.format(self.base)) as f:
			self.j = json.load(f)
		self.m = {}
	...
	def extract(self):
		...
		for line in self.lines:
			...
			if root.nsmap:
				...
				prefix = ''
			try:
				title = root.xpath('//{0}titleStmt/{0}title'.format(prefix), namespaces=namespace)[0].text.strip()
			except IndexError:
				title = 'No Title'
			try:
