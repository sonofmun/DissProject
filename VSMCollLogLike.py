...
import numpy as np
CollVSMDir = askdirectory(title = 'Where are your collocate VSMs located?')
LLVSMDir = askdirectory(title = 'Where would you like to save your log likelihood VSM files?')
CollVSMList = os.listdir(CollVSMDir)
for File in CollVSMList:
    print(File)
    CollVSMFile = os.path.join(CollVSMDir, File)
    name = File.split('_')[0]
    LLVSMFile = os.path.join(LLVSMDir, ''.join([name, '_LL.txt']))
