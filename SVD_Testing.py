...
def RunTests(size, e_min, e_max, orig=None):
    e_range = [x/100 for x in range(int(e_min*100), int(e_max*100)+1, 25)]
    if orig == None:
    ...
    for file in files:
        ...
        ll_list = []
        ll_svd_dict = {}
        [ll_svd_dict.update({x: []}) for x in e_range]
        pmi_list = []
        pmi_svd_dict = {}
        [pmi_svd_dict.update({x: []}) for x in e_range]
        counter = 1
        for train, test in kf:
            ...
            '''print('Starting LL calculations at %s' %
                  (datetime.datetime.now().time().isoformat()))
                            ...
                            (np.log
                                ...
                                (t_ll.ix[ind_int,ind_int],
                                 t_test.ix[ind_int,ind_int])
                                  .values))
                            * exponent))
            ll_U, ll_S = svd_calc(t_ll)
            del t_ll
            '''
            print('Starting PPMI calculations at %s' %
                  (datetime.datetime.now().time().isoformat()))
            t_pmi = PPMI(t_train).PPMI()
            t_pmi[t_pmi<=0] = .0000001
                            ...
                            (np.log
                                ...
                                (t_pmi.ix[ind_int,ind_int],
                                 t_test.ix[ind_int,ind_int])
                                  .values))
                            * exponent)
            pmi_U, pmi_S = svd_calc(t_pmi)
            del t_pmi
            for svd_exp in e_range:
                print('Starting SVD calculations for '
                      'exponent=%s at %s' %
                      (svd_exp,
                       datetime.datetime.now().time().isoformat()))
                pmi_e = pd.DataFrame(np.dot(pmi_U, pmi_S**svd_exp),
                             index=t_train.index,
                             columns=t_train.columns)
                pmi_e[pmi_e<=0] = .0000001
                pmi_svd_dict[svd_exp].append(np.sum
                            (np.log
                                (1/np.multiply
                                (pmi_e.ix[ind_int,ind_int],
                                 t_test.ix[ind_int,ind_int])
                                  .values))
                            * exponent)
                print(np.sum
                ...
                del pmi_e
                '''
                ll_e = pd.DataFrame(np.dot(ll_U, ll_S**svd_exp),
                             index=t_train.index,
                             columns=t_train.columns)
                ll_svd_dict[svd_exp].append(pow
                           (np.e,
                            np.sum
                            (np.log
                                (1/np.multiply
                                (ll_e.ix[ind_int,ind_int],
                                 t_test.ix[ind_int,ind_int])
                                  .values))
                            * exponent))
                del ll_e
        ...
        for e_val in e_range:
            '''perplex_dict[('LL_SVD_%s' % (e_val),
                          size,
                          'lems=%s' % (lemmata),
                          'weighted =%s' % (weighted))] = \
                            sum(ll_svd_dict[e_val])/len(ll_svd_dict[e_val])
            '''
            perplex_dict[('PPMI_SVD_%s' % (e_val),
                          size,
                          'lems=%s' % (lemmata),
                          'weighted =%s' % (weighted))] = \
                            sum(pmi_svd_dict[e_val])/len(pmi_svd_dict[e_val])
        dest_file = file.replace('.txt',
                                 '_%s_%s_perplexity.pickle' % (e_min, e_max))
