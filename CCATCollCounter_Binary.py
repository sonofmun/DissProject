...
for filename in fileslist:
    ...
    dest_file = '/'.join([coll_dir, '_'.join([filename.split('_')[1], 'binary_coll.pickle'])])
    dict_file = '/'.join([coll_dir, '_'.join([filename.split('_')[1], 'binary_coll_dict.pickle'])])
    ...
    for x in range(N):
        ...
        for y in window:
            try:
                colls.append(wordlist[x+y])                
            except IndexError as E:
                continue
        for coll in set(colls):
            coll_df.ix[lemma, coll] += 1
        if x % 10000 == 0:
            print(x)
